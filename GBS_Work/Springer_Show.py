'''
The Springer Show:
A script for reading GBS files and assigning parentage to alleles

Written by J. Hayes; last editted 1/9/2017

Purpose: This script reads .tab files generated by GBS analysis, and attempts to map
each read to the respective parent. Currently it is geared towards analysis of chickpeas

To modify for other systems, where parents may be located in different columns; when working
with other systems modify the following:
	Line 52: the entry range should be changed to grab the appropriate part of the sample name
	Line 78: Change index to omit parent columns as needed
	Line 156, 157: Change index to include parent columns; may need to add column labels if 
	additional replicates are present

Input: The script reads a .tab file, given as the first argument when running the script
Output: The script generates the following files 
(located in the .tab file directory):
	1) Offspring_Allele_Counts.csv - contains the frequency count for each allele at a given position 
	2) Parsed_Allele_Counts.csv - lists the frequencies of the top 2 alleles for each location
	3) Unique_Sites.csv - lists all useful parent sites, where the reads are the same length for both parents
	4) Insertion_Sites.csv - lists all useful parent sites, where the read length is different for both parents
	5) Identical_Sites.csv - lists all parent sites where P1 = P2
	6) Non_Matching_Sites.csv - lists sites where either P1_1 != P1_2 or P2_1 != P2_2
	7) Unassigned_Alleles.csv - lists cases where one of the top two alleles (or both) cannot be assigned to a parent
	8) Assigned_Offspring_Reads.csv - The main output; takes the tab file and assigns a parent to each read

Usage: python Springer_Show.py tab_file.tab
...
def File_Reader(file):
	#Text to show program is running:
	print("Finding some interesting guests for the show...")
	...
	for entry in df.columns:
		column_labels.append(entry[4:17]) ###Here, 4:17 is the range where the sample name is
...
def Allele_Counter(df):
	#Text to show program is running:
	print("Counting all the alleles...")

	#Meat and potatoes
	df = df.ix[:, :-4] #change index as needed to omit parent reads
	...
	Allele_Counter_File_Writer(data_dict, "./Output_Files/Offspring_Allele_Counts.csv")
	return data_dict
...
def Min_Max_Analyzer(data_dict):
	#Text to show program is running
	print("Finding the top two expressions...")
	...
	Allele_Counter_File_Writer(allele_analysis, './Output_Files/Parsed_Allele_Counts.csv')
...
def Parent_Info_Stripper(df):
	#Text to show program is running
	print("Stripping the parents down (it's not as dirty as it sounds)...")

	#Relabel the parent columns for easy access; will need to generalize
	parents = df.ix[:, -4:] #Change as needed to index the parent columns
	parent_labels = ['P1-1', 'P1-2', 'P2-1', 'P2-2'] #Change as needed if more than 2 replicates per parent
...
def Parent_Stat_Reader(parents):
	print("Here's some interesting stats about the parents: ")
...
def Allele_Assigner(allele_dict, parents_dict):
	#Print out to show the program is working
	print('Jerry: And the father is...')
	...
	Site_File_Writer(unassigned_alleles, './Output_Files/Unassigned_Alleles.csv')
...
def Read_Assignment(df, assigned_alleles):
	...
	print("Jerry! Jerry! Jerry! Assigning the reads")
...
if len(sys.argv) == 2:
	if isfile(sys.argv[1]):
		...
		print("Sorry, that's not a file...please try again, specifying the tab file you'd like to analyze")
elif len(sys.argv) > 2:
	print("Too many variables on the dance floor...please enter only one argument after calling the script")
elif len(sys.argv) < 2:
	print("Whoops, you need to tell me there the .tab file is")